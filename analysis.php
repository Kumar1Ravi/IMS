...
// On page load, check URL param 'tab' and activate that tab if present
document.addEventListener('DOMContentLoaded', () => {
    const urlParams = new URLSearchParams(window.location.search);
    const tabParam = urlParams.get('tab');
    if (tabParam) {
        const tabElement = Array.from(document.querySelectorAll('.tab')).find(t => t.textContent.trim().toLowerCase().replace(/\s/g, '') === tabParam.toLowerCase());
        const tabContentElement = document.getElementById(tabParam);
